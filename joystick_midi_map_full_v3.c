...
/* joystick_midi_map_full_v3.c
 *
 * Integra lógica mejorada para joysticks y gatillos:
 * - Joysticks: 4 direcciones básicas (arriba, abajo, izquierda, derecha)
 * - Gatillos: Interpretados como un solo botón presionado
 * - Mantiene la lógica para otros botones
 */
...
void export_event_log() {
    FILE *fp = fopen("joystick_log_v3.txt", "w");
    if (fp == NULL) {
    ...
    fclose(fp);
    printf("Log exportado a 'joystick_log_v3.txt'\n");
}
...
int main() {
    ...
    printf("Logger HID V3 iniciado. Presione Ctrl+C para salir.\n");
