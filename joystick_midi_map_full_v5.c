/* joystick_midi_map_full_v5.c
 *
 * Mejoras aplicadas:
 * - Corrige la mezcla de botones al presionar
 * - Elimina mensajes de "liberado" para botones y gatillos
 * - Evita mezcla entre LT y RT
 * - Mantiene la eliminación del "Neutro" en joysticks
 */
...
void interpret_hid_input(const unsigned char *buf) {
    ...
    // Gatillo Izquierdo (LT) - sin mezcla con RT y sin "liberado"
    if (lt > 0 && !lt_pressed) {
        lt_pressed = 1;
    ...
    // Gatillo Derecho (RT) - sin mezcla con LT y sin "liberado"
    if (rt > 0 && !rt_pressed) {
        rt_pressed = 1;
...
int main() {
    ...
    printf("Logger HID V5 iniciado. Presione Ctrl+C para salir.\n");
