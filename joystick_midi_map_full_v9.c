/* joystick_midi_map_full_v9.c
 *
 * Mejoras aplicadas:
 * - Gatillos LT y RT detectados correctamente (basado en buf[7])
 * - Umbral de presión aplicado (≥ 0x15)
 * - Se mantiene la lógica de botones comunes
 * - Evita lecturas múltiples durante la presión
 */
...
#define BUF_SIZE 65
#define DEADZONE 8
#define TRIGGER_THRESHOLD 0x15  // Umbral mínimo para considerar presionado
...
void interpret_hid_input(const unsigned char *buf) {
    ...
    uint8_t rx = buf[3];
    uint8_t ry = buf[4];
    uint8_t control_byte = buf[7]; // Para LT y RT
    uint8_t trigger_pressure = buf[8]; // Nivel de presión

    // Leer botones desde buf[5] y buf[6]
    uint16_t button_state = (buf[6] << 8) | buf[5];

    // Detectar cambios en botones comunes
    for (int i = 0; i < 16; i++) {
        int mask = 1 << i;
    ...
    // Gatillo Izquierdo (LT) - buf[7] == 0x01
    if (control_byte == 0x01) {
        if (trigger_pressure >= TRIGGER_THRESHOLD && !lt_pressed) {
            lt_pressed = 1;
            printf("Gatillo Izquierdo (LT) presionado\n");
        } else if (trigger_pressure < TRIGGER_THRESHOLD && lt_pressed) {
            lt_pressed = 0;
        }
    }

    // Gatillo Derecho (RT) - buf[7] == 0x02
    if (control_byte == 0x02) {
        if (trigger_pressure >= TRIGGER_THRESHOLD && !rt_pressed) {
            rt_pressed = 1;
            printf("Gatillo Derecho (RT) presionado\n");
        } else if (trigger_pressure < TRIGGER_THRESHOLD && rt_pressed) {
            rt_pressed = 0;
        }
    }
}
...
int main() {
    ...
    printf("Logger HID V9 iniciado. Presione Ctrl+C para salir.\n");
