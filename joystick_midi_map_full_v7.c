...
/* joystick_midi_map_full_v7.c
 *
 * Mejoras aplicadas:
 * - Umbral de presión para gatillos (≥ 0x80)
 * - Evita lecturas múltiples durante la presión
 * - Separa completamente LT y RT en la lógica
 * - Mantiene eliminación del "Neutro" y omite "liberado" en botones/gatillos
 */
...
void interpret_hid_input(const unsigned char *buf) {
    ...
    uint8_t rx = buf[3];
    uint8_t ry = buf[4];
    uint8_t lt = buf[8];  // Gatillo Izquierdo
    uint8_t rt = buf[7];  // Gatillo Derecho
...
int main() {
    ...
    printf("Logger HID V7 iniciado. Presione Ctrl+C para salir.\n");
